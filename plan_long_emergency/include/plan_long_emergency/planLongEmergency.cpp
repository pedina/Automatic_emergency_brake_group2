...
public:
    // FSR-05: Érzékenységi szintek tárolása
    struct SafetyLimits {
        double max_accel; // m/s^2 (abszolút érték)
        double max_jerk;  // m/s^3 (abszolút érték)
    };

    PlanLongEmergency() : Node("plan_long_emergency") {
        // Paraméterek deklarálása
        this->declare_parameter("safety_distance", 5.0); // 5 méter a kép alapján
        ...
        // Feliratkozások (Architektúra ábra szerint)
        ...
        sub_strategy_ = this->create_subscription<std_msgs::msg::String>(
            "plan/strategy", 10, [this](const std_msgs::msg::String::SharedPtr msg) { 
                current_strategy_ = msg->data; 
            });
...
private:
    void onTimer() {
        // Csak akkor tervezünk, ha a behavior planner kéri (strategy: longEmergencyControl)
        if (current_strategy_ == "longEmergencyControl") {
            // FSR-05: Aktuális mód kiválasztása
            uint8_t mode = current_behavior_->deceleration_mode.data;
            if (mode_limits_.find(mode) == mode_limits_.end()) mode = 1; // Default: Normal
    ...
    autoware_planning_msgs::msg::Trajectory generateTrajectory(SafetyLimits lims) {
        ...
        // FSR-01: Lassulás számítása (v^2 = 2as -> a = v^2 / 2s)
        double a_req = -(v_ego * v_ego) / (2.0 * s_stop);

        // Korlátozás a táblázat szerinti módhoz (Saturation)
        if (a_req < -lims.max_accel) {
            a_req = -lims.max_accel;
        }

        // Időlépcsős trajektória generálása (TSR-07/08)
        double horizon = this->get_parameter("prediction_horizon").as_double();
        double dt = 0.1; // Trajektória pontok felbontása
        double curr_v = v_ego;
        double curr_s = 0.0;
        ...
        for (double t = 0; t <= horizon; t += dt) {
            ...
            // Egyszerűsített 1D előrevetítés a trajektóriában
    ...
    rclcpp::Subscription<std_msgs::msg::String>::SharedPtr sub_strategy_;
    ...
    std::string current_strategy_ = "noAction";
